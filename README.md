...
- 命中 `PROXY` 的网站：`本程序 -> FastLink 127.0.0.1:7892 -> Wi-Fi`
...
## 为什么不要开启 FastLink 的系统代理

如果让 FastLink 自己全局接管系统代理，FastLink 内部的 `DIRECT` 连接仍可能由 FastLinkCore 发起，这会让本应走 Windows 默认有线路由的流量继续从 Wi-Fi 发出。这个项目的设计是：

1. 浏览器先通过系统 PAC 进入本程序的本地分流代理。
2. 只有命中 `PROXY` 的目标才转发给 FastLink `127.0.0.1:7892`。
...
6. 保持 FastLink 本地 HTTP 代理可用：`127.0.0.1:7892`。
